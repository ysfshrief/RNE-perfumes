...
export default function LoginPage() {
  ...
  const showError = (code) => {
    ...
    setError(msg ? msg[lang] || msg.en : (lang === "ar" ? "حصل خطأ، حاول تاني" : "Something went wrong"));
  };
  ...
  const googleSignIn = async () => {
    ...
    if (res.redirecting) return; // page redirects to Google
    setLoading(false);
      ...
      <div className={styles.card}>
        ...
        <form onSubmit={submit} className={styles.form}>
          <div className={styles.field}>
            ...
            <input type="email" required value={form.email} onChange={set("email")} dir="ltr" />
          </div>
          <div className={styles.field}>
            <label>{t("auth.password")}</label>
            <input type="password" required value={form.password} onChange={set("password")} dir="ltr" />
          </div>
